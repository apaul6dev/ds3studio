...
import { CONFIG_NAME } from "src/app/shared/constants";
...
export class InicioComponent implements OnInit {
  ...
  constructor( public appSettings: AppSettings, private gelocationService: GelocationService, 
    private inicioService: InicioService, private zone: NgZone, private router: Router){
  ...
  getConfigAppStart() {
    ...
    const configuracion = sessionStorage.getItem(CONFIG_NAME)

    this.config = JSON.parse(configuracion!);
  ...
  obtenerConfiguracion() {
    ...
    this.inicioService.getConfiguracion().subscribe(rs=>{
      console.log('Configuracion: ', rs);
      if (rs) {
        this.config = rs;
        sessionStorage.setItem(CONFIG_NAME, JSON.stringify(this.config));
      }
      this.zone.run(() => {
  ...
  cambiarEstado() {
    ...
    this.inicioService.smartboxactionalarma(this.coordenadas).subscribe(rs=>{
      console.log('cambiando estado: smartboxactionalarma', rs);
  ...
  presionar(accionboton: string) {
    console.log('presionar', accionboton);
    /*
    if (!accionboton) {
      return;
    }
  ...
  soltar(accionboton: any) {
    console.log('soltar', accionboton);
